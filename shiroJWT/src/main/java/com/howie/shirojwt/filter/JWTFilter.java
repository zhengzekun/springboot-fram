...
public class JWTFilter extends BasicHttpAuthenticationFilter {
     ...
     */
    private void responseError(ServletResponse response, int message) {
        try {
            HttpServletResponse httpServletResponse = (HttpServletResponse) response;
            //设置编码，否则中文字符在重定向时会变为空字符串
//            message = URLEncoder.encode(message, "UTF-8");
//            httpServletResponse.sendRedirect("/unauthorized/" + message);
//            httpServletResponse.sendError(message);
            switch (message) {
                case 1:
                    httpServletResponse.sendError(1001);
                    break;
                case 2:
                    httpServletResponse.sendError(1002);
                case 3:
                    httpServletResponse.sendError(1003);
            }
        } catch (IOException e) {
